...
import WeatherContentColumn from '../weatherContentColumn/WeatherContentColumn.js';
...
import './style.css';
 ...
 const WeatherContent = ({data}) => {
    ...
    const setDaysOfWeek = () => {
        ...
        nextDayDate.setHours(12, 0, 0, 0); //setting 12:00 of next day
       ...
       return daysOfweek.map(day => <WeatherContentColumn
                                        key={day.dt}
                                        dayWeather={day}
                                        tempUnit={tempUnit}/>);
    };

    const setTempUnitValue = () => tempUnit === 'Celsius' ? setTempUnit('Fahrenheit') : setTempUnit('Celsius');

    return (
        <>
            <Row>
                <Col md={{ size: 3, offset: 9 }} className="button-wrap">
                    <Button onClick={setTempUnitValue}>{tempUnit === 'Celsius' ? 'Fahrenheit' : 'Celsius'}</Button>
                </Col>
            </Row>
